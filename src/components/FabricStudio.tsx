...
export function FabricStudio({ onContinue }: { onContinue?: (selection: FabricSelection) => void }) {
  ...
  async function analyze() {
    ...
    try {
      const response = await fetch("/api/fabric/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: file.name, contentType: file.type, size: file.size }) });
  ...
  return <section className="fabricStudio">
    <div className="studioIntro"><p className="eyebrow">FABRIC INTELLIGENCE · V2</p><h1>Start with the cloth.</h1><p>Upload one clear fabric photo. LLinen Earth estimates visible characteristics, then asks you to confirm the real fibre family because composition cannot be reliably proven from an image alone.</p><div className="studioSteps" aria-label="Fabric design process"><span className="active">01 Upload</span><span>02 Read fabric</span><span>03 Confirm composition</span><span>04 Build brief</span></div></div>

    <div className="studioGrid"><div className="fabricCanvas"><div className="canvasHead"><div><span className="micro">YOUR FABRIC</span><h2>{file ? "Fabric captured" : "Show us the material"}</h2></div><span className="privacyTag">Private session asset</span></div>
      <label className={`uploadStage ${preview ? "hasImage" : ""}`} onDragOver={(e) => e.preventDefault()} onDrop={onDrop}><input type="file" accept="image/*" onChange={onInput} />{preview ? <><img src={preview} alt="Uploaded fabric preview" /><div className="uploadShade" /><div className="replaceHint">Click or drop another image to replace</div></> : <div className="uploadEmpty"><span className="uploadPlus">+</span><h3>Drop a fabric photo here</h3><p>or choose from your device · camera works on mobile</p><small>Best result: even light, fabric fills the frame, no heavy filters.</small></div>}</label>
      ...
      <div className="canvasActions"><div>{fileMeta ?? "PNG, JPG, WEBP · up to 10 MB"}</div><button className="button light" disabled={!file || status === "analyzing"} onClick={analyze}>{status === "analyzing" ? "Reading fabric…" : profile ? "Analyze again" : "Analyze fabric"}</button></div>{error && <p className="studioError">{error}</p>}
    </div>

    <aside className="analysisPanel"><div className="analysisHead"><div><span className="micro">FABRIC PROFILE</span><h2>{profile ? "What we can see" : "Waiting for fabric"}</h2></div><span className={`analysisState ${status}`}>{status === "complete" ? "Estimated" : status === "analyzing" ? "Analyzing" : "Not started"}</span></div>
      {!profile ? <div className="analysisEmpty"><div className="orb" /><p>Your structured fabric profile will appear here.</p><ul><li>Material family estimate</li><li>Color and pattern</li><li>Surface and drape cues</li><li>Climate direction</li></ul></div> : <><p className="analysisSummary">{profile.summary}</p><div className="paletteRow">{profile.palette.map((color) => <span key={color} style={{ background: color }} title={color} />)}</div><div className="observationList">{profile.observations.map((item) => <div className="observation" key={item.label}><div><span>{item.label}</span><strong>{item.value}</strong></div><div className="confidence"><i style={{ width: `${Math.round(item.confidence * 100)}%` }} /><em>{Math.round(item.confidence * 100)}%</em></div></div>)}</div>
        ...
        <div className="cautionBox"><strong>Estimated from photo</strong>{profile.cautions.map((x) => <p key={x}>{x}</p>)}</div><button className="continueButton" type="button" onClick={continueWithFabric}>Judge this fabric for my occasion <span>→</span></button><p className="devNote">Visual analysis estimates appearance only. Confirming Linen, Cotton, TR/PV, TR-Wool or another known stock family lets the Designer use the correct performance model.</p>
      </>}
    </aside></div>
  </section>;
}
